...
export class RefreshTokenInterceptor implements NestInterceptor {
    ...
    intercept( context: ExecutionContext, next: CallHandler<any> ): Observable<any> | Promise<Observable<any>> {
        ...
        if (useCookie && body?.refreshToken !== undefined) {
            this.logger.warn( `[${method}] ${url} - Both useCookie=true and body.refreshToken provided`);
        ...
        if (body?.refreshToken) {
            ...
            if (!refreshTokenFromCookies) {
                this.logger.warn(`[${method}] ${url} - No refresh token in cookies or body`);
                throw new UnauthorizedException('Refresh Token Expired');
            }

            const preview = refreshTokenFromCookies.slice(0, 8) + '...';
            this.logger.log(`[${method}] ${url} - Refresh token provided in cookie: ${preview}`);
